...
class IBApi(EWrapper,EClient):
    ...
    def accountSummary(self, reqId: int, account: str, tag: str, value: str, currency: str):
        with open ('Accounts_Summary.txt', 'a+') as file:  # append, create if doesn't exist
            file.write ("%s, %s, %s, %s\n" % (account, tag, value, currency))
    # after finishing the above function
    ...
    def historicalDataEnd(self, reqId, start, end):
        print(reqId)
    #fucntion to get the contracts details
    def contractDetails(self, reqId, contractDetails):
        print (reqId, contractDetails.contract.secType)
        #write contracts details in a text file
        with open ('Contracts.txt', 'a+') as file:  # append, create if doesn't exist
            file.write ("ContractDetails. ReqId: %s,Symbol: %s,Sectype: %s,ConId: %s,@ %s\n" % (reqId, contractDetails.summary.symbol,
                                            contractDetails.summary.secType,contractDetails.summary.conId,
                                            contractDetails.summary.exchange))
    #called at the end of the previous function to show if it worked well
...
class Bot:
    ...
    def __init__(self):
        ...
        self.ib.reqContractDetails (4444, contract)
        #requests account summary
        self.ib.reqAccountSummary (9002, "All", "$LEDGER")
        self.ib.reqIds(-1)
        # Request Market Data
